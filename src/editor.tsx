...
export default () => {
	...
	const handleAddSection = async () => {
		const s = await $prompt("Section name:")
	...
	const openSection = async (sectionName: string) => {
		sectionContent.current.value =
			(await axios.post(
				"/api/read",
				{ gameName: gameName.current.value, sectionName },
			)).data
					...
					<List>
						...
						{sectionList.map((x) => (
							<ListItem button key={x} onClick={() => openSection(x)}>
								{x}
								<ListItemSecondaryAction>
									<IconButton color="secondary" onClick={() => deleteSection(x)}>
										<Icon>delete</Icon>
									</IconButton>
								</ListItemSecondaryAction>
							</ListItem>
